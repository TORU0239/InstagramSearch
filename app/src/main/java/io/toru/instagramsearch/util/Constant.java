package io.toru.instagramsearch.util;

/**
 * Created by wonyoung on 2017. 5. 21..
 */

public class Constant {
    public static final String INSTAGRAM_BASE_URL = "https://www.instagram.com";


    // DetailActivity Intent Key
    public static final String KEY_MODEL_LIST = "key_model_list";
    public static final String KEY_MODEL_SEARCHED_ID = "key_model_searched_id";
    public static final String KEY_MODEL_IS_MORE_AVAILABLE= "key_model_is_more_available";
    public static final String KEY_MODEL_CURRENT_ITEM_ID= "key_model_current_item_id";

}
